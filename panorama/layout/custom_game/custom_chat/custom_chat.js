...
function AddHeroInfoToLine(player_id, text) {
	const player_info = Game.GetPlayerInfo(player_id);
	...
	text += `<panel class='HeroBadge PlusHeroBadgeIconSmall ${rank_class}' selectionpos='auto' />`;
	text += `<img class='HeroIcon' selectionpos='auto' src='${GetPortraitImage(player_id, player_info.player_selected_hero)}' />`;
...
function CreateCustomMessage(data) {
	...
	if (data.sender_id > -1) {
		text = AddHeroInfoToLine(data.sender_id, text);
		text += `${!!data.is_team ? `[${allies_tag}] ` : NON_BREAKING_SPACE}`;
		text += `${GetPlayerNameWithTag(data.sender_id)}: `
